...
import org.wustrive.dao.jdbc.SqlParameter;
		...
		try {
			while (iterator.hasNext()) {
				...
				for (Method method : methods) {
					if (StrUtil.equals(method.getName().toLowerCase(), "set" + key.toLowerCase())) {
						String parameterType = method.getParameterTypes()[0].getSimpleName();
						if (StrUtil.equals("String", parameterType)) {
							method.invoke(bean, Convert.toStr(map.get(key)));
						} else if (StrUtil.equals("Integer", parameterType)) {
							method.invoke(bean, Convert.toInt(map.get(key)));
						} else if (StrUtil.equals("Long", parameterType)) {
							method.invoke(bean, Convert.toLong(map.get(key)));
						} else if (StrUtil.equals("Double", parameterType)) {
